...
flowchart TB
 ...
 subgraph DecisionMakers["Decision Makers (决策层)"]
    ...
    Supervisor -- "6. Autopilot Planning (自动规划)" --> Scheduler
    Scheduler -- "7. Optimized Schedule (优化排程)" --> App
...
*   **特性**：
    ...
    *   **零等待自动流转 (Zero-Wait Auto-Flow)**：完工即刻自动排程下一步，最小化等待时间（拉动式生产）。
...
## 📝 更新日志摘要
*   **v8.2.0**: **智能排程 2.0** - "零等待"自动流转 (Auto-Flow)，简化优先级逻辑 (红标加分/+1000) 并优化默认权重 (50/50)。**仪表盘** - 增强排序 (红 > 黄 > 日期)。**操作界面** - 优化页眉目标数量显示。批量编辑稳定性修复。
