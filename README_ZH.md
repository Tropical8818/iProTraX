...
> 🤖 **V6.3.0 新功能**: **超级编辑模式** (管理员/主管可用)、完成逻辑增强 (N/A 视为完成)、手动列宽调整。
...
### V6.3.0
- ✏️ **超级编辑模式**: 管理员和主管现在可以直接在表格中修改详情列（WO ID, PN等），无需重新导入 Excel。
- ✅ **完成逻辑增强**: 最后一个步骤标记为 "N/A" 现在的工单会被正确视为已完成。
- 📐 **手动列宽调整**: 支持拖动表头边缘调整列宽，并自动保存您的偏好。
- 🛡️ **ECD 保护**: 在超级编辑模式下，ECD (预计完成日期) 列保持锁定以保护计算逻辑。
### V6.1.0
